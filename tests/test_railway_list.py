...
It also double-wrapped their return values ({"projects": {"projects":
...
def _install_bridge_stubs():
    ...
    # Mirror real shapes: each returns the GraphQL 'data' dict directly.
...
def test_railway_list_without_project_id(monkeypatch):
    ...
    # Returned as-is: no extra {"projects": ...} wrapper layer.
    assert out == {
...
def test_railway_list_with_project_id(monkeypatch):
    ...
    def fake_list_services(pid):
        calls["pid"] = pid
        return {"project": {"services": {"edges": [{"node": {"id": "s1"}}]}}}
    ...
    assert out == {"project": {"services": {"edges": [{"node": {"id": "s1"}}]}}}
